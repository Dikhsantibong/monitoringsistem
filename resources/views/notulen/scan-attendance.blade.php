            ...
            <div class="max-w-md mx-auto">
                <div class="divide-y divide-gray-200">
                    <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                        <h2 class="text-2xl font-bold mb-8 text-center text-gray-800">Form Absensi Notulen</h2>
                        <p class="text-center text-gray-600 mb-4">{{ $notulen->agenda }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('attendance-form').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('{{ url("/public/notulen/{$notulen->id}/attendance") }}', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                window.location.href = '{{ route("notulen.show", $notulen->id) }}';
            } else {
                showError(data.message || 'Gagal menyimpan absensi');
            }
        } catch (error) {
            console.error('Submit Error:', error);
            showError('Terjadi kesalahan saat menyimpan absensi');
        }
    });
