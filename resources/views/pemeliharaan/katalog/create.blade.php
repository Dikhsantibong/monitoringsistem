    ...
    <div class="flex-1 main-content">
        <header class="bg-white shadow-sm sticky top-0">
            <div class="flex justify-between items-center px-6 py-3">
                <div class="flex items-center gap-x-3">
                    ...
                    <h1 class="text-xl font-semibold text-gray-800">Buat Katalog Baru</h1>
                </div>
        ...
        <main class="px-6 pt-6">
            <div class="bg-white rounded-lg shadow p-6 w-full">
                <div class="mb-4">Silakan isi form PDF di bawah ini, lalu klik Simpan.</div>
                <div class="w-full h-[80vh] flex justify-center items-center">
                    <iframe id="pdfjs-viewer" src="{{ asset('pdf.js/web/viewer.html') }}?file={{ $templateUrl }}&hideSaveBtn=1" style="width:100%;height:100%;border:none;"></iframe>
                </div>
                ...
                <div class="mt-4 flex justify-end">
                    <button id="savePdfBtn" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Simpan Katalog</button>
                </div>
            </div>
        </main>
    </div>
...
window.addEventListener('message', function(event) {
    if (event.data && event.data.type === 'save-pdf' && event.data.data) {
        ...
        if (blob) {
            ...
            .then(data => {
                if (data.success) {
                    alert('Katalog berhasil disimpan!');
