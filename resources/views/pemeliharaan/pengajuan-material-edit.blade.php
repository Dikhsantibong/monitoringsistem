    ...
    <div class="flex-1 main-content">
        <header class="bg-white shadow-sm sticky top-0">
            <div class="flex justify-between items-center px-6 py-3">
                <h1 class="text-xl font-semibold text-gray-800">Edit Pengajuan Material</h1>
            </div>
        </header>
        <main class="px-6 pt-6">
            <div class="bg-white rounded-lg shadow p-6 w-full">
                <div class="mb-4">Edit file PDF pengajuan material di bawah ini, lalu klik Simpan untuk memperbarui.</div>
                <div class="w-full h-[80vh] flex justify-center items-center">
                    <iframe id="pdfjs-viewer" src="{{ asset('pdf.js/web/viewer.html') }}?file={{ $pdfUrl }}&hideSaveBtn=1" style="width:100%;height:100%;border:none;"></iframe>
                </div>
...
window.addEventListener('message', function(event) {
    if (event.data && event.data.type === 'save-pdf' && event.data.data) {
        ...
        if (blob) {
            ...
            fetch("{{ route('pemeliharaan.pengajuan-material.update', $file->id) }}", {
                method: 'POST',
