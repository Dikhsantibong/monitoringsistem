    ...
    public function handle(ScoreCardDailyUpdated $event)
    {
        try {
            ...
            $data = [
                ...
                'ketentuan_rapat' => $event->scoreCard->ketentuan_rapat,
            ...
            Log::info("Score Card Daily sync to UP Kendari successful", [
                'action' => $event->action,
                'source_unit' => $event->sourceUnit,
                'score_card_id' => $event->scoreCard->id
            ]);

        } catch (\Exception $e) {
            Log::error("Score Card Daily sync to UP Kendari failed", [
