...
use App\Events\ScoreCardWeeklyUpdated;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class SyncScoreCardWeeklyToUpKendari
{
    public function handle(ScoreCardWeeklyUpdated $event)
    {
        try {
            ...
            switch($event->action) {
                case 'create':
                    $upKendariDB->table('score_card_weekly')->insert($data);
                    break;
                case 'update':
                    $upKendariDB->table('score_card_weekly')
            ...
            Log::info("Score Card Weekly sync successful", [
                'action' => $event->action,
                'source_unit' => $event->sourceUnit,
                'id' => $event->scoreCard->id
            ]);
        } catch (\Exception $e) {
            Log::error("Score Card Weekly sync failed", [
