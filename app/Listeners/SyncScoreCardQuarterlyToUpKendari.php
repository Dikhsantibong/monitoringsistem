...
use App\Events\ScoreCardQuarterlyUpdated;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class SyncScoreCardQuarterlyToUpKendari
{
    public function handle(ScoreCardQuarterlyUpdated $event)
    {
        try {
            ...
            switch($event->action) {
                case 'create':
                    $upKendariDB->table('score_card_quarterly')->insert($data);
                    break;
                case 'update':
                    $upKendariDB->table('score_card_quarterly')
            ...
            Log::info("Score Card Quarterly sync successful", [
                'action' => $event->action,
                'source_unit' => $event->sourceUnit,
                'id' => $event->scoreCard->id
            ]);
        } catch (\Exception $e) {
            Log::error("Score Card Quarterly sync failed", [
