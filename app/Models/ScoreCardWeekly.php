...
class ScoreCardWeekly extends Model
{
    use HasFactory;

    protected $table = 'score_card_weekly';
    ...
    protected static function boot()
    {
        ...
        static::created(function ($scoreCard) {
            if (!static::$isSyncing) {
                ...
                event(new ScoreCardWeeklyUpdated($scoreCard, 'create'));
        ...
        static::updated(function ($scoreCard) {
            if (!static::$isSyncing) {
                static::$isSyncing = true;
                event(new ScoreCardWeeklyUpdated($scoreCard, 'update'));
                static::$isSyncing = false;
            }
        });
        static::deleted(function ($scoreCard) {
            if (!static::$isSyncing) {
                static::$isSyncing = true;
                event(new ScoreCardWeeklyUpdated($scoreCard, 'delete'));
                static::$isSyncing = false;
            }
        });
    }
