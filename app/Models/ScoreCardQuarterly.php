...
class ScoreCardQuarterly extends Model
{
    use HasFactory;

    protected $table = 'score_card_quarterly';
    ...
    protected static function boot()
    {
        ...
        static::created(function ($scoreCard) {
            if (!static::$isSyncing) {
                ...
                event(new ScoreCardQuarterlyUpdated($scoreCard, 'create'));
        ...
        static::updated(function ($scoreCard) {
            if (!static::$isSyncing) {
                static::$isSyncing = true;
                event(new ScoreCardQuarterlyUpdated($scoreCard, 'update'));
                static::$isSyncing = false;
            }
        });
        static::deleted(function ($scoreCard) {
            if (!static::$isSyncing) {
                static::$isSyncing = true;
                event(new ScoreCardQuarterlyUpdated($scoreCard, 'delete'));
                static::$isSyncing = false;
            }
        });
    }
