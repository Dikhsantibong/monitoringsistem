    ...
    public function edit($id)
    {
        try {
            ...
            $workOrder = (object) [
                ...
                'worklocation' => $woRaw->worklocation ?? '-',
                'siteid' => $woRaw->siteid ?? '-',
                'orgid' => $woRaw->orgid ?? '-',
                'downtime' => $woRaw->downtime ?? '-',
                'risk' => $woRaw->risk ?? '-',
                'environment' => $woRaw->environment ?? '-',
                'backoutplan' => $woRaw->backoutplan ?? '-',
                'justifypriority' => $woRaw->justifypriority ?? '-',
                // People
                ...
                'matcostellipse' => $woRaw->matcostellipse ?? 0,
                'servcostellipse' => $woRaw->servcostellipse ?? 0,
                'totalcostellipse' => $woRaw->totalcostellipse ?? 0,
                'matcostwo' => $woRaw->matcostwo ?? 0,
                'servcostwo' => $woRaw->servcostwo ?? 0,
                'totalcostwo' => $woRaw->totalcostwo ?? 0,
                // Codes
                'jpnum' => $woRaw->jpnum ?? '-',
                'pmnum' => $woRaw->pmnum ?? '-',
                'failurecode' => $woRaw->failurecode ?? '-',
                'problemcode' => $woRaw->problemcode ?? '-',
                'problemcodepln' => $woRaw->problemcodepln ?? '-',
                'actioncodepln' => $woRaw->actioncodepln ?? '-',
                'causecodepln' => $woRaw->causecodepln ?? '-',
                'failurecodepln' => $woRaw->failurecodepln ?? '-',
                ...
                'woeq7' => is_numeric($woRaw->woeq7 ?? null) ? number_format($woRaw->woeq7, 2) : ($woRaw->woeq7 ?? '-'),
                'woeq8' => $woRaw->woeq8 ?? '-',
                'woeq9' => $woRaw->woeq9 ?? '-',
                'woeq10' => $woRaw->woeq10 ?? '-',
                'woeq11' => $woRaw->woeq11 ?? '-',
                'woeq12' => is_numeric($woRaw->woeq12 ?? null) ? number_format($woRaw->woeq12, 2) : ($woRaw->woeq12 ?? '-'),
                'woeq13' => $fmtDate($woRaw->woeq13 ?? null),
                'woeq14' => is_numeric($woRaw->woeq14 ?? null) ? number_format($woRaw->woeq14, 2) : ($woRaw->woeq14 ?? '-'),
                // PLN Custom
