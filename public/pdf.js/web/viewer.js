  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', async function() {
      if (window.PDFViewerApplication && window.PDFViewerApplication.pdfDocument) {
        ...
        // PDF.js >= v2.10.377: gunakan saveDocument()
        let data;
        if (typeof pdfDoc.saveDocument === 'function') {
          data = await pdfDoc.saveDocument();
        } else {
          // fallback: ambil dari url asli
          const response = await fetch(window.PDFViewerApplication.url);
          data = await response.arrayBuffer();
        }
        // Kirim ke parent
        window.parent.postMessage({ type: 'save-pdf', data }, '*');
      } else {
        alert('PDF belum dimuat sepenuhnya.');
      }
    });
  }
});
